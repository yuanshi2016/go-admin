...
// @Success 200 {object} response.Response	"{"code": 200, "message": "后台处理中.. 请稍候查看状态"}"
// @Router /api/v1/sys-contract-token/transfer [post]
// @Security Bearer
func (e SysContractToken) Transfer(c *gin.Context) {
	s := service.SysContractToken{}
	req := dto.SysContractTokenTransferReq{}
	err := e.MakeContext(c).
		MakeOrm().
		Bind(&req).
...
// @Success 200 {object} response.Response{data=response.Page{list=[]models.SysContractTokenJoin}} "{"code": 200, "data": [...]}"
...
func (e SysContractToken) GetPage(c *gin.Context) {
	...
	p := actions.GetPermissionFromContext(c)
	list := make([]models.SysContractTokenJoin, 0)
	var count int64
...
// @Tags 合约token
// @Param id path string false "id"
// @Success 200 {object} response.Response{data=models.SysContractTokenJoin} "{"code": 200, "data": [...]}"
...
func (e SysContractToken) Get(c *gin.Context) {
	...
	var object models.SysContractTokenJoin
